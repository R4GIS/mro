...
opts_chunk$set(fig.align='center', fig.height = 5,
...
\bigskip
<<anscombe, out.width='.65\\textwidth'>>=
library(tidyr)
library(dplyr)
library(ggplot2)
ans <- anscombe
...
The first step is to \href{https://mran.revolutionanalytics.com/download/}{download Microsoft R Open}.\footnote{\href{https://mran.revolutionanalytics.com/download/} {https://mran.revolutionanalytics.com/download/}} We assume that the reader has a basic familiarity with the R language, but beginning R users can type in the examples here and run them, without any prior knowledge. This text focuses on graphical functions; for a general introduction to R, the Microsoft R Portal\index{Microsoft R Portal} provides \href{https://mran.revolutionanalytics.com/documents/getting-started/}{a great list of resources for getting started.}\footnote{\href{https://mran.revolutionanalytics.com/documents/getting-started/}{https://mran.revolutionanalytics.com/documents/getting-started/}} For data visualization, it is particularly important to understand how to work with data frames, vectors, and matrices, and to master the factor class. R Help can be easily accessed by entering a question mark before any R command or function in the console, for example: \texttt{?barplot}. \index{R Help}Online R resources abound; \href{http://stackoverflow.com}{Stack Overflow}\footnote{\href{http://stackoverflow.com}{http://stackoverflow.com}} has become the go-to source for answers to R questions.\index{Microsoft R Open|)}
...
\bigskip
<<barplot, fig.height = 4, echo = FALSE, out.width='.8\\textwidth'>>=
n <- getfigopts()
...
\bigskip
<<boxplot, fig.height = 4.5, out.width='.9\\textwidth'>>=
n <- getfigopts()
# Box plots
par(mar = c(5,9,4,2))
data <- read.csv("data/countries2012.csv")
...
\bigskip
<<monthplot, fig.height = 4.5, fig.width = 8.5, out.width='.8\\textwidth'>>=
n <- getfigopts()
# Month plot
par(mar=c(5,6,2,2))
...
\bigskip
<<scatterplot, fig.width = 5, out.width='.6\\textwidth'>>=
n <- getfigopts()
data <- read.csv("data/countries2012.csv")
...
\bigskip
<<scatterplotlabel, fig.width = 5, out.width='.6\\textwidth'>>=
n <- getfigopts()
...
\bigskip
<<colorscatterplot, fig.width=5, out.width='.6\\textwidth'>>=
n <- getfigopts()
...
\bigskip
<<parallel2, out.width='.8\\textwidth'>>=
n <- getfigopts()
...
<<parallelchange, echo = TRUE, eval = FALSE>>=
parcoord(data[,c("GDP","LIFEEXP","CHMORT","TFR")],
@
...
\bigskip
<<barpercentcode, echo = FALSE, eval = TRUE>>=
...
\bigskip
<<pie2by2, fig.height =4, fig.width=5.5, out.width='.7\\textwidth'>>=
n <- getfigopts()
data <- read.table("data/fathers.txt")
colors4 <- c("lightblue", "skyblue3", "rosybrown1",
             "rosybrown3")
...
<<horizdividedbarchart, fig.height = 4.5, out.width='.85\\textwidth'>>=
n <- getfigopts()
par(mar=c(5, 8, 4, 2))
...
<<groupedbarchart, out.width='.8\\textwidth'>>=
n <- getfigopts()
fathers <- read.table("data/fathers.txt")
...
<<panelpercent, fig.height=3, out.width='.9\\textwidth'>>=
n <- getfigopts()
library(ggplot2)
library(tidyr)
fathers <- read.table("data/fathers.txt")
...
\bigskip
<<living, fig.height=4, out.width='.9\\textwidth'>>=
n <- getfigopts()
library(ggplot2)
...
\bigskip
<<themetextgg, out.width='.9\\textwidth'>>=
library(ggplot2)
...
<<likertliving, fig.width = 8, out.width='.9\\textwidth'>>=
n <- getfigopts()
library(HH)
library(tidyr)
library(dplyr)
